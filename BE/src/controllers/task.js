...
const createTask = async (req, res) => {
  try {
    ...
    const existingUser = await userModel.findOne({ _id: assignedTo });
...
const getTasksByStatus = async (req, res) => {
  try {
      ...
      if (req.headers.role == "user") filters.assignedTo = req.headers.userId;
      if (status)  filters.status = status;
console.log("getTasksByStatus", filters);
      let tasks;
